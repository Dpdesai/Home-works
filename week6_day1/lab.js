...
const bubble_sort = list => {
    for (let i = 0; i < list.length - 1; i++) {             // n times
        for (let j = 0; j < list.length - 2; j++) {         // n times
...
const isPalindrome = input => {
    const stack = [];                           // O(1)
    let output = "";                            // O(1)
    for (let i = 0; i < input.length; i++){     // n times
      stack.push(input[i]);                     // O(1)
    }
    while (stack.length) {                      // n times
...
const printAllNumbersThenSumPairs = numArray => {
    ...
    numArray.forEach((num, index)=>{            // n times
